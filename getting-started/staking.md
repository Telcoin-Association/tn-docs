...
1. **Governance Approval** - Governance issues a ConsensusNFT to whitelist your validator address
2. **Stake** - You submit your BLS public key, proof of possession, and stake amount
3. **Activate** - Once synced, you submit an activation transaction to join the committee
...
* `--address` - Your validator's Ethereum address (receives block rewards)
...
You will be prompted to enter a passphrase to encrypt your BLS key. Alternatively, set the passphrase via:
...
1. Submit your validator address to governance for approval
...
You can verify your whitelist status by checking if you own a ConsensusNFT at the registry address.

### Step 3: Stake

Once whitelisted, submit your stake transaction to the ConsensusRegistry.
...
* `blsPubkey` - Your compressed BLS public key (96 bytes)
...
**Value:** Send the exact stake amount required by the current stake configuration.

#### Reading Your Keys

Your BLS public key and proof of possession are stored in `node-info.yaml` after key generation:
...
After staking, your validator status changes to `Staked`.

### Step 4: Start and Sync Your Node

Start your validator node and wait for it to sync with the network.
...
Use the `tn_syncing` RPC method to check if your node is synced:
...
Wait until your node reports it is fully synced with the current epoch before proceeding.

### Step 5: Activate

Once synced, submit an activation transaction to signal readiness for committee inclusion.
...
This function must be called from your validator address (the address that owns the ConsensusNFT and submitted the stake).
...
1. Your status changes to `PendingActivation`
2. At the next epoch boundary, your status changes to `Active`
3. You become eligible for committee selection in subsequent epochs
